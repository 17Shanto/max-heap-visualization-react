import { useMemo } from "react";
import { HeapItem, calculateTreeLayout } from "@/utils/heapUtils";

interface TreeVisualizationProps {
  heap: HeapItem[];
  highlightedIndices: number[];
  phase: string;
  message: string;
}

const NODE_RADIUS = 28;

const TreeVisualization = ({
  heap,
  highlightedIndices,
  phase,
  message,
}: TreeVisualizationProps) => {
  const width = 900;
  const height = 600;

  const { nodes, edges } = useMemo(
    () => calculateTreeLayout(heap, width, height),
    [heap],
  );

  const getNodeClasses = (index: number) => {
    const isHighlighted = highlightedIndices.includes(index);
    if (isHighlighted && (phase === "extracting" || phase === "heapify-down")) {
      return {
        fill: "hsl(var(--node-extracting))",
        textFill: "hsl(var(--node-extracting-foreground))",
        glow: true,
      };
    }
    if (isHighlighted) {
      return {
        fill: "hsl(var(--node-highlight))",
        textFill: "hsl(var(--node-highlight-foreground))",
        glow: true,
      };
    }
    return {
      fill: "hsl(var(--node))",
      textFill: "hsl(var(--node-foreground))",
      glow: false,
    };
  };

  return (
    <div className="flex flex-col h-full">
      <h2 className="text-sm font-display font-semibold uppercase tracking-wider text-muted-foreground mb-2">
        Max-Heap Tree
      </h2>

      {/* Status message */}
      <div className="mb-2 px-3 py-2 rounded-lg bg-muted/50 border border-border min-h-[40px] flex items-center">
        <p className="text-xs font-mono text-muted-foreground leading-relaxed">
          {message || "Idle — add items and press Play to start"}
        </p>
      </div>

      {/* Tree SVG */}
      <div className="flex-1 rounded-xl bg-card border border-border overflow-hidden flex items-center justify-center">
        {heap.length === 0 ? (
          <p className="text-muted-foreground text-sm">No items in heap</p>
        ) : (
          <svg
            viewBox={`0 0 ${width} ${height}`}
            className="w-full h-full"
            preserveAspectRatio="xMidYMid meet"
          >
            <g>
              {/* Edges */}
              {edges.map((edge) => (
                <line
                  key={`edge-${Math.round(edge.parentX)}-${Math.round(edge.parentY)}-${Math.round(edge.childX)}-${Math.round(edge.childY)}`}
                  x1={edge.parentX}
                  y1={edge.parentY}
                  x2={edge.childX}
                  y2={edge.childY}
                  stroke="hsl(var(--edge-color))"
                  strokeWidth={2}
                  strokeLinecap="round"
                />
              ))}
            </g>

            <g>
              {/* Nodes */}
              {nodes.map((node) => {
                const style = getNodeClasses(node.index);
                return (
                  <g key={node.item.id}>
                    {/* Glow effect */}
                    {style.glow && (
                      <circle
                        cx={node.x}
                        cy={node.y}
                        r={NODE_RADIUS + 6}
                        fill="none"
                        stroke={style.fill}
                        strokeWidth={3}
                        opacity={0.3}
                        className="animate-pulse"
                      />
                    )}

                    {/* Node circle */}
                    <circle
                      cx={node.x}
                      cy={node.y}
                      r={NODE_RADIUS}
                      fill={style.fill}
                      className="transition-all duration-300"
                    />

                    {/* Weight */}
                    <text
                      x={node.x}
                      y={node.y - 4}
                      textAnchor="middle"
                      dominantBaseline="middle"
                      fill={style.textFill}
                      fontSize={14}
                      fontWeight={700}
                      fontFamily="'JetBrains Mono', monospace"
                    >
                      {node.item.weight}
                    </text>

                    {/* Name */}
                    <text
                      x={node.x}
                      y={node.y + 12}
                      textAnchor="middle"
                      dominantBaseline="middle"
                      fill={style.textFill}
                      fontSize={8}
                      fontFamily="'Space Grotesk', sans-serif"
                      opacity={0.85}
                    >
                      P{node.item.personId}
                    </text>
                  </g>
                );
              })}
            </g>
          </svg>
        )}
      </div>
    </div>
  );
};

export default TreeVisualization;
